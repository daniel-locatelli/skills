...
The skill's spine is a four-class rule for acceptance numbers — invariant, banded statistic, identity, not-portable — and the discipline of asking which class a number is *before* asserting it. It exists because the specification shipped three numbers that a correct implementation fails, each surviving review because nobody ran it against the reference. Also documents why bow-tie plates on saddles are forced by the Dupin indicatrix rather than a bug, why a developable cylinder produces the best-looking and most worthless numbers of any test surface, and why a tangent-plane-intersection ring is planar only where its solve succeeded — so planarisation before fabrication is mandatory, not a refinement.
