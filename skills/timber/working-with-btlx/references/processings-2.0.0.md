# BTLx 2.0.0 — processing parameter reference

> GENERATED from `BTLx_2_0_0.xsd` by `scripts/generate_reference.py` — do not hand-edit.
...
50 processings. Every processing also carries the
...
| `EndLimited` | `BooleanType` (string): enum: yes, no | no | no |  |
| `Length` | `LengthType` (double): 0.0 … 100000.0 | 0.0 | no |  |
| `AngleRefStart` | `AngleType` (double): 0.1 … 179.9 | 90.0 | no |  |
| `AngleRefEnd` | `AngleType` (double): 0.1 … 179.9 | 90.0 | no |  |
| `AngleOppStart` | `AngleType` (double): 0.1 … 179.9 | 90.0 | no |  |
| `AngleOppEnd` | `AngleType` (double): 0.1 … 179.9 | 90.0 | no |  |
...
| `MachiningLimits` | inline complex | — | no |  |
...
| `ConeAngle` | double: 0.0 … 30.0 | — | no |  |
| `UseFlankAngle` | `BooleanType` (string): enum: yes, no | no | no |  |
| `FlankAngle` | double: 5 … 35 | 15.0 | no |  |
| `Shape` | `TenonShapeType` (string): enum: automatic, square, round, rounded, radius | automatic | no |  |
| `ShapeRadius` | `LengthSType` (double): 0.0 … 1000.0 | 20.0 | no |  |
...
> Definition of a Japanese tenon
...
| `ConeAngle` | double: 0.0 … 30.0 | 0.0 | no |  |
...
|---|---|---|---|---|
| `@ToolID` | unsignedInt | — | no |  |
| `Contour` | complex → see `SimpleContourType` | — | yes |  |
...
| `@ForGlueing` | `BooleanType` (string): enum: yes, no | — | no |  |
...
> Defintion of a free NURBS surface
...
| `VariantParameter` | complex → see `VariantParameterType` | — | yes |  |
...
| Parameter | Type / range | Default | Required | Notes |
|---|---|---|---|---|
| `@Inclination` | `InclinationType` (double): -89.9 … 89.9 | — | no |  |
| `@Recess` | `ContourRecessType` (string): enum: automatic, noPassOver, passOverStart, passOverEnd, passOverAll | — | no |  |
| `@Process` | `BooleanType` (string): enum: yes, no | — | no |  |
| `@FaceLimited` | `BooleanType` (string): enum: yes, no | — | no | only for saw and mill contour |
| `@NailSpacing` | double | — | no | only for nail, cramp and screw contour |
| `@CrampAngleRef` | ? | — | no | only for cramp contour |
| `@CrampAngle` | ? | — | no | only for cramp contour |
| `@ScrewDiameter` | ? | — | no | only for screw contour |
| `EndPoint` | complex → see `CoordinateType` | — | yes |  |

## `NURBSCurveType`

> Defintion of a NURBS curve
